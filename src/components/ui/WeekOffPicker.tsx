...
// Off-day picker with two modes -- mirrors the CRM's week-off-picker.tsx so a
// client's submission here maps 1:1 onto the same mandate fields a recruiter
// sees internally:
...
export default function WeekOffPicker({
  ...
  return (
    <div>
      <label className="block text-[13px] font-medium text-slate-700 mb-1.5">Off day(s)</label>
      <div className="flex gap-2 mb-2">
        <button
          type="button"
          onClick={() => setType('fixed')}
          className={`flex-1 rounded-xl border px-3 py-2 text-[13px] font-medium transition-colors ${
            value.week_off_type === 'fixed' ? 'border-blue-400 bg-blue-50 text-blue-700' : 'border-slate-300 text-slate-600'
          }`}
        >
          Fixed days off
        </button>
        <button
          ...
          className={`flex-1 rounded-xl border px-3 py-2 text-[13px] font-medium transition-colors ${
            value.week_off_type === 'rotational' ? 'border-blue-400 bg-blue-50 text-blue-700' : 'border-slate-300 text-slate-600'
          }`}
        >
          Rotational off
        </button>
      ...
      {value.week_off_type === 'fixed' && (
        <div className="grid grid-cols-4 gap-1.5 rounded-xl border border-slate-200 p-3">
          {WEEK_DAYS.map((day) => (
            <label key={day} className="flex items-center gap-1.5 text-[13px] text-slate-700">
      ...
      {value.week_off_type === 'rotational' && (
        <div className="rounded-xl border border-slate-200 p-3 space-y-3">
          <div>
            <p className="text-[12px] text-slate-500 mb-1">Rotational offs per week</p>
            <div className="flex gap-2">
              {([1, 2] as const).map((n) => (
                <label
                  key={n}
                  className={`flex-1 flex items-center justify-center gap-1.5 rounded-xl border px-3 py-2 text-[13px] cursor-pointer ${
                    value.rotational_offs_per_week === n ? 'border-blue-400 bg-blue-50 text-blue-700' : 'border-slate-300 text-slate-600'
                  }`}
                >
                  <input
                    type="radio"
